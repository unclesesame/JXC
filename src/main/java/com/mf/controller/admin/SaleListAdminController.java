...
public class SaleListAdminController {
	...
	public Map<String,Object> countSaleByDay(String begin,String end)throws Exception{
		...
		List<String> dates=DateUtil.getRangeDates(begin, end);//要这个dates的目的 是为了展示页面
		List<Object> ll=saleListService.countSaleByDay(begin, end);//返回多个list 每个list有三个字段
		//返回类型为obj 可以把这个返回值设置一个pojo类
		for(String date:dates){//遍历每天
			SaleCount sc=new SaleCount();
			sc.setDate(date);//先塞日期
			boolean flag=false;
			for(Object o:ll){//每天进销（因为sql根据日期分的组）
				Object []oo=(Object[]) o;//把list的每个值 强转成obj数组
				String dd=oo[2].toString().substring(0,10);//截取前10个字符  也就是日期（很有可能没有值）
				if(dd.equals(date)){ // 确定日期和sql语句的日期是否相同   2确定dd是否有值
